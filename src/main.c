...
uint16_t toggleTime = 32768; //LED on and off period based on 16Mhz clock with 256 prescaler
...
int main(void) {
    ...
    // Debugging the setup
    //uart_print("--- System Debug Report ---\r\n");
    // Initialize UART at 9600 baud
    uart_init(9600);
    uart_print("UART Initialized (Pure C)\r\n");
    uart_print("UART Initialized (Pure C)\r\n");
    uart_debug_binary8("TCCR1A", TCCR1A);
    uart_debug_binary8("TCCR1B", TCCR1B);
    uart_debug_binary16("OCR1A", OCR1A);
